...
class UnetAudioSeparator:
    ...
    def __init__(self, model_config):
        ...
        self.num_initial_filters = model_config.get("num_initial_filters",24)
        self.filter_size = model_config.get("filter_size",15)
        self.merge_filter_size = model_config.get("merge_filter_size",5)
        self.input_filter_size = model_config.get("input_filter_size",15)
        self.output_filter_size = model_config.get("output_filter_size",1)
        self.upsampling = model_config.get("upsampling", 'learned')
        self.output_type = model_config.get("output_type", 'direct')
        self.context = model_config.get("context", False)
        self.padding = "valid" if model_config.get("context", False) else "same"
        self.source_names = model_config.get("source_names", ['enhancement'])
        self.num_channels = 1 if model_config.get("mono_downmix", True) else 2
        self.output_activation = model_config.get("output_activation", 'identity')
    ...
    def get_output(self, input, training, return_spectrogram=False, reuse=True):
        ...
        with tf.variable_scope("separator", reuse=reuse):
            ...
            for i in range(self.num_layers):
                current_layer = tf.layers.conv1d(current_layer, self.num_initial_filters + (self.num_initial_filters * i), self.filter_size, strides=1, activation=unet_utils.LeakyReLU, padding=self.padding) # out = in - filter + 1
                enc_outputs.append(current_layer)
                current_layer = current_layer[:,::2,:] # Decimate by factor of 2 # out = (in-1)/2 + 1

            current_layer = tf.layers.conv1d(current_layer, self.num_initial_filters + (self.num_initial_filters * self.num_layers),self.filter_size,activation=unet_utils.LeakyReLU,padding=self.padding) # One more conv here since we need to compute features after last decimation
            ...
            for i in range(self.num_layers):
                ...
                if self.upsampling == 'learned':
                    ...
                    current_layer = InterpolationLayer.learned_interpolation_layer(current_layer, self.padding, i)
                else:
                    if self.context:
                ...
                current_layer = unet_utils.crop_and_concat(enc_outputs[-i-1], current_layer, match_feature_dim=False)
                current_layer = tf.layers.conv1d(current_layer, self.num_initial_filters + (self.num_initial_filters * (self.num_layers - i - 1)), self.merge_filter_size,
                                                 activation=unet_utils.LeakyReLU,
                                                 padding=self.padding)  # out = in - filter + 1

            current_layer = unet_utils.crop_and_concat(input, current_layer, match_feature_dim=False)
            ...
            if self.output_activation == "identity":
                ...
                out_activation = lambda x: unet_utils.AudioClip(x, training)
            else:
            ...
            if self.output_type == "direct":
                ...
                cropped_input = unet_utils.crop(input,current_layer.get_shape().as_list(), match_feature_dim=False)
